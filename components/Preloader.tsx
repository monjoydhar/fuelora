...
export function Preloader() {
  ...
  useEffect(() => {
    ...
    const alreadyLoaded = localStorage.getItem("fuelora-preloader-seen");
    ...
    const update = () => {
      ...
      const target =
        docReady && fontsReady ? 100 : Math.min(94, current + 1.5);

      current += (target - current) * 0.08;

      if (target === 100 && current > 99.5) {
        current = 100;
      }
    ...
    const finish = () => {
      ...
      localStorage.setItem("fuelora-preloader-seen", "1");
  ...
  return (
    <div
      ref={ref}
      className="preloader-panel fixed inset-0 z-[200] bg-[var(--brown)] text-[var(--cream)]"
    ...
    >
      <div className="absolute inset-x-10 top-10 flex items-center justify-between text-[10px] font-bold uppercase tracking-[0.32em] text-[var(--tan)] md:inset-x-12 md:top-12">
        <span className="preloader-copy">
          FUELORA / PERFORMANCE NUTRITION
        </span>

        <span className="preloader-copy hidden md:block">
